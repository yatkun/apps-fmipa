...
<div class="main-content">
    ...
    <form wire:submit.prevent="generate">
                    ...
                    <div class="row">
                        @foreach ($template->placeholders as $placeholder)
                            <div class="mb-3 col-md-6">
                                <label for="{{ $placeholder }}" class="form-label">{{ ucfirst(str_replace('_', ' ', $placeholder)) }}</label>
                                <input type="text" class="form-control" id="{{ $placeholder }}" wire:model.defer="formData.{{ $placeholder }}">
                                @if (isset($placeholderHints[$placeholder]) && !empty($placeholderHints[$placeholder]))
                                    <small class="text-muted">Contoh : {{ $placeholderHints[$placeholder] }}</small>
                                @endif
                            </div>
                        @endforeach
                    </div>
        ...
        @if ($template->dynamic_table_marker && !empty($template->table_placeholders))
            <div class="mb-4 card">
                <div class="card-body d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" wire:click="addTableRow" class="btn btn-sm btn-primary">Tambah Baris</button>
                </div>
                <div class="card-body">
                                    ...
                                    @foreach ($template->table_placeholders as $ph)
                                        <th>
                                            ...
                                            @if (isset($placeholderHints[$ph]) && !empty($placeholderHints[$ph]))
                                                <br><small class="text-muted">Contoh : {{ $placeholderHints[$ph] }}</small>
                                            @endif
                                        </th>
                                    @endforeach
                            ...
                            <tbody>
                                @foreach ($tableData as $index => $rowData)
                                    <tr>
                                        ...
                                        @endforeach
                                        <td>
                                            <button type="button" wire:click="removeTableRow({{ $index }})" class="btn btn-sm btn-danger">X</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
        ...
        @if ($generatedFilePath)
            <a href="{{ Storage::url($generatedFilePath) }}" target="_blank" class="mt-4 btn btn-info ms-2" download>Unduh Surat</a>
        @endif
    </form>
</div>
